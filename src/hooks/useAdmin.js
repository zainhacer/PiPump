  ...
  useEffect(() => {
    async function fetch() {
      const [config, tokens, users, trades] = await Promise.all([
        ...
        supabase.from('trades').select('pi_amount, fee_amount, type, created_at')
          .eq('status', 'completed'),
      ])
      ...
      const todayVolume   = todayTrades.reduce((s, t) => s + parseFloat(t.pi_amount), 0)
      const weekVolume    = weekTrades.reduce((s, t) => s + parseFloat(t.pi_amount), 0)
      const totalFees     = tradeList.reduce((s, t) => s + parseFloat(t.fee_amount || 0), 0)
      const newUsersToday = userList.filter(u => new Date(u.created_at) >= today).length
...
export function useAdminTokens(filter = 'all', search = '') {
  ...
  useEffect(() => {
    ...
    let query = supabase
      .from('tokens')
      .select(`
        id, name, ticker, image_url, status, is_featured,
        is_verified, current_price, volume_total,
...
export const adminActions = {
  ...
  async toggleFeatured(tokenId, current) {
    const { error } = await supabase
      .from('tokens').update({ is_featured: !current }).eq('id', tokenId)
    if (error) throw error
  },

  async toggleVerified(tokenId, current) {
    const { error } = await supabase
      .from('tokens').update({ is_verified: !current }).eq('id', tokenId)
    if (error) throw error
  },
  ...
  async toggleBan(userId, current) {
    const { error } = await supabase
      .from('users').update({ is_banned: !current }).eq('pi_uid', userId)
    if (error) throw error
    // Also suspend their tokens if banning
    if (!current) {
  ...
  async toggleAdmin(userId, current) {
    const { error } = await supabase
      .from('users').update({ is_admin: !current }).eq('pi_uid', userId)
    if (error) throw error
  },
  ...
  async resolveReport(reportId, action) {
    const { error } = await supabase
      .from('reports').update({ status: action }).eq('id', reportId)
    if (error) throw error
  },
  ...
  async updateConfig(updates) {
    const { error } = await supabase
      .from('platform_config').update({ ...updates, updated_at: new Date().toISOString() })
